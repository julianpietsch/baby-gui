function extractCellDataStacksParfor(cTimelapse)
...
ntps = find(cTimelapse.timepointsProcessed,1,'last');
...
for t=1:numel(timepoints)
    ...
    for c=1:numel(trap)
        cellInd = find(trapInfo(trap(c)).cellLabel==cells(c),1);
        if ~isempty(cellInd)
...
%                     bdryNorm = convn(ones(size(cellImg3)),nucFilt3,'same');
%                     nucConv3 = convn(cellImg3,nucFilt3,'same')./bdryNorm;
